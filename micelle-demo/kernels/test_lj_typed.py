"""Step 3: Per-type LJ force kernel with Lorentz-Berthelot epsilon combining.

Builds on test_lj_neighbors (passes sim + hw). Adds:
- Per-atom epsilon array (own_eps) packed in cell layout
- Pairwise eps_ij = sqrt(eps_i * eps_j) via broadcast+transpose
- sigma = 1.0 for all types (standard CG simplification)

DFB count: 25 (under 28 limit, room for Coulomb later).
"""
import numpy as np
import torch
import ttnn
...
def build_cell_data(positions, eps_per_atom, box_length, r_cut):
    ...
    return own_px, own_py, own_pz, own_eps, masks, cell_atom_map, n_cells_total, n_cells_dim


def make_lj_typed_kernel(c_n_dim, c_dim2, c_box, c_inv_box):
    """Per-type LJ with eps_ij = sqrt(eps_i * eps_j). 25 DFBs."""
    c_half = 0.5
    c_lj_scale = 24.0

    @ttl.kernel(grid="auto")
    def lj_typed_kernel(own_px, own_py, own_pz, own_eps,
                        self_mask, scaler,
        ...
        # Input DFBs: own (4) + neighbor (4) + mask + scaler = 10
        ...
        # Total: 10 + 4 + 4 + 3 + 6 = 27 DFBs (under 32 limit)
                        ...
                        with sc_cb.wait() as sc:
                            for nbr_i in range(N_NBR):
                                with ex_cb.wait() as ex, ey_cb.wait() as ey, ez_cb.wait() as ez, ee_cb.wait() as e_eps, sm_cb.wait() as sm:
                                    ...
                                    # PBC z-displacement + self-exclusion
                                    with ba_cb.reserve() as ba:
                                    ...
                                    # Pairwise eps: sqrt(eps_i * eps_j) via broadcast+transpose
                                    with ba_cb.reserve() as ba:
                                    ...
                                    with ba_cb.wait() as ei, bb_cb.wait() as ej:
                                        with fm_cb.reserve() as eps_ij:
                                            eps_ij.store(ttl.math.sqrt(ei * ej))

                                    # LJ force: 24*eps_ij/r^2 * (2/r^12 - 1/r^6)
                                    with r2_cb.wait() as r2, fm_cb.wait() as eps_pair:
                                    ...
                                    # Project, reduce (dims=[1]), accumulate
    ...
    return lj_typed_kernel


def reference_lj_lb(positions, eps_per_atom, box_length):
    """O(N^2) reference LJ with Lorentz-Berthelot eps combining, sigma=1."""
    ...
    return np.sum(f_mag[:, :, None] * dr, axis=1)
...
def test_lj_typed():
    ...
    # Assign types with different epsilon
    ...
    own_px, own_py, own_pz, own_eps, masks, cell_atom_map, n_cells_total, n_cells_dim = \
        build_cell_data(positions, eps_per_atom, box_length, r_cut)
    ...
    kernel = make_lj_typed_kernel(c_n_dim, c_dim2, float(box_length), 1.0/float(box_length))
    print("Running per-type LJ kernel...")
